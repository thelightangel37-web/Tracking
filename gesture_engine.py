...
TRACK_CONFIDENCE: float    = 0.55   # high tracking confidence prevents landmark hallucination
...
def compute_palm_normal_3d(world_lm_np: Optional[np.ndarray], handedness_str: str = "Right") -> tuple[tuple[float, float, float], str, bool]:
    ...
    Integrates Left/Right handedness for cross-product direction stability.
    Classifies posture mode with corrected MediaPipe vertical coordinate axes (+Y is down).
    """
    ...
    # MediaPipe coordinate space: +Y points downwards.
    # Therefore, ventral normal pointing down towards the ground (palm down) means ny > 0.40.
    is_flat_down = ny > 0.40

    # Reinforce palm-down detection with middle finger direction (should point forward/away along -Z axis)
    if is_flat_down and len(world_lm_np) >= 13:
        v_middle = world_lm_np[12, :3] - world_lm_np[9, :3]
        if v_middle[2] > 0.05:  # points back towards user instead of forward/away
            is_flat_down = False
...
def check_finger_curl(lm_np: np.ndarray, world_lm_np: Optional[np.ndarray] = None, is_flat_mode: bool = False) -> tuple[float, float]:
    ...
    # Index finger extension score: 0.60 -> 0.82 maps to 0.0 -> 1.0 (natural pointing / index extension angle)
    index_score = clamp((index_cos - 0.60) / (0.82 - 0.60))

    # Pointing / Dwell start score is directly driven by index finger extension
    pointing_score = index_score
    if math.isnan(pointing_score):
        pointing_score = 0.0
...
def normalize_frame_lighting(small_frame: np.ndarray) -> np.ndarray:
    """
    Downscaled selective lighting normalization running on 320x240 frame.
    Evaluates mean luminance and contrast; applies CLAHE only under extreme
    overexposure (mean > 180) or low contrast glare. Passes through raw frame
    under normal lighting to save CPU cycles.
    """
    try:
...
class HandLockTracker:
    """
    Selects the hand closest to the camera/screen (largest 2D palm span metric)
    and statefully locks onto it with hysteresis and spatial tracking continuity.
    """
    def __init__(self, hysteresis_margin: float = 0.20, spatial_dist_thresh: float = 0.35, miss_grace_s: float = 0.25):
        self.hysteresis_margin = hysteresis_margin      # candidate hand must be >1.20x larger to switch lock
        self.spatial_dist_thresh = spatial_dist_thresh  # max normalized dist for matching same hand
        self.miss_grace_s = miss_grace_s                # hold lock during brief frame drops
        
        self.locked_center: Optional[tuple[float, float]] = None
        self.locked_span: float = 0.0
        self.last_seen_ts: float = 0.0

    def select_hand(
        self,
        all_lms: List[Any],
        all_world_lms: Optional[List[Any]],
        all_handedness: List[str],
        now: float
    ) -> tuple[Optional[Any], Optional[Any], str]:
        if not all_lms:
            if self.locked_center is not None and (now - self.last_seen_ts) > self.miss_grace_s:
                self.reset()
        ...
        for i, lms in enumerate(all_lms):
            if not lms or len(lms) < 10:
                continue
            w_x, w_y = lms[0].x, lms[0].y
            m_x, m_y = lms[9].x, lms[9].y
            span = math.hypot(m_x - w_x, m_y - w_y)
            center = ((w_x + m_x) / 2.0, (w_y + m_y) / 2.0)

            wl = all_world_lms[i] if (all_world_lms and i < len(all_world_lms)) else None
            h_str = all_handedness[i] if (all_handedness and i < len(all_handedness)) else "Right"
            candidates.append({
                'lm': lms,
                'wl': wl,
                'handedness': h_str,
        ...
        if self.locked_center is None or (now - self.last_seen_ts) > self.miss_grace_s:
            best = max(candidates, key=lambda c: c['span'])
            self.locked_center = best['center']
            self.locked_span = best['span']
            self.last_seen_ts = now
            return best['lm'], best['wl'], best['handedness']

        same_hand_cand = None
        min_d = float('inf')
        for c in candidates:
            d = math.hypot(c['center'][0] - self.locked_center[0], c['center'][1] - self.locked_center[1])
            if d < self.spatial_dist_thresh and d < min_d:
                min_d = d
                same_hand_cand = c

        largest_cand = max(candidates, key=lambda c: c['span'])

        if same_hand_cand is not None:
            if largest_cand is not same_hand_cand and largest_cand['span'] > same_hand_cand['span'] * (1.0 + self.hysteresis_margin):
                selected = largest_cand
            else:
                selected = same_hand_cand
        else:
            selected = largest_cand

        self.locked_center = selected['center']
        self.locked_span = selected['span']
        self.last_seen_ts = now
        return selected['lm'], selected['wl'], selected['handedness']

    def reset(self) -> None:
        self.locked_center = None
        self.locked_span = 0.0
...
class GestureProcessor:
    ...
    def process(self, lms: Any, world_lms: Optional[Any] = None, handedness: str = "Right") -> None:
        ...
        raw_lm_np = np.nan_to_num(raw_lm_np, nan=0.0, posinf=1.0, neginf=0.0)

        world_lm_np = None
        if world_lms:
            world_lm_np = np.array([[lm.x, lm.y, getattr(lm, 'z', 0.0)] for lm in world_lms], dtype=np.float32)
            world_lm_np = np.nan_to_num(world_lm_np, nan=0.0, posinf=1.0, neginf=0.0)
    ...
    def _run_pipeline(self, now: float, raw_lm_np: Optional[np.ndarray], world_lm_np: Optional[np.ndarray], handedness: str = "Right") -> None:
        ...
        if raw_lm_np is not None:
            ...
            if self._last_ts > 0 and self._last_raw_lm is not None:
                ...
                if dt > 0:
                    ...
                    # Foreshortening ratio metric (Index span vs Palm span)
                    ...
                    # Dynamic EMA Alpha bound: clamp max alpha to prevent raw landmark twitching during motion
            ...
            # ── Foreshortening Palm Direction Vector Lock ──
            # When pointing directly at screen (rf_ratio < 0.35), lock index finger vector
            # along stable palm axis (Wrist 0 -> Index MCP 5) to prevent 2D vector flipping
        ...
        # ── Cursor position anchored directly to Index Fingertip (LM 8) ────────
        tip_x = float(smooth_lm_np[8, 0])
        tip_y = float(smooth_lm_np[8, 1])

        # Screen anchor for skeleton and cursor
        skel_anchor_x = tip_x
        skel_anchor_y = tip_y
        raw_x = tip_x
        raw_y = tip_y

        # Sanitize raw_x, raw_y against NaN / Inf before feeding to OneEuro filter
        if math.isnan(raw_x) or math.isinf(raw_x): raw_x = tip_x
        if math.isnan(raw_y) or math.isinf(raw_y): raw_y = tip_y
        ...
        if math.isnan(smooth_x) or math.isinf(smooth_x): smooth_x = tip_x
        if math.isnan(smooth_y) or math.isinf(smooth_y): smooth_y = tip_y
        ...
        # Compute pose-invariant 3D metric depth factor (wrist depth & 3D world landmark span)
        if world_lm_np is not None and len(world_lm_np) >= 10:
            m_span = float(np.linalg.norm(world_lm_np[9, :3] - world_lm_np[0, :3]))
            raw_depth_z = m_span / 0.10  # baseline metric 3D wrist-to-middle MCP span (~10cm)
        else:
        ...
        # ── 8. OS mouse control & Click Injection ──────────────────
        ...
        if cursor_state == "CLICK" and self._prev_cursor_state != "CLICK":
            if self._mouse is not None and _Button is not None:
                ...
                # Fallback to xdotool mousemove + click with DISPLAY environment set
                def _inject_xdotool_click(sx: int, sy: int):
                    try:
                        ...
                        log.info("xdotool click injected at (%d, %d)", sx, sy)
                    except Exception as ex:
                        log.warning("xdotool click failed: %s", ex)
...
class CameraStream:
    ...
    def start(self) -> bool:
        ...
        # Hardware camera auto-exposure control attempt
        try:
            self.cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)  # Manual / driver-dependent mode
        except Exception:
            pass
    ...
    with _mp_vision.HandLandmarker.create_from_options(options) as landmarker:
        ...
        while not stop_event.is_set():
            try:
                ...
                # Constant high-speed 30 FPS processing rate (prevents 8-FPS death spiral)
                ...
                if not ret or frame is None:
                    ...
                    if read_fail_count > 30:
                        raise RuntimeError(
                            f"Camera returned {read_fail_count} consecutive bad frames — likely disconnected"
                        )
                    continue
                ...
                # 4x Faster Pipeline: Downscale via INTER_AREA first, then run gated light normalization
                ...
                # Watchdog: If in_flight has been True for > 150ms without callback, reset it!
                now_ts = time.perf_counter()
                if in_flight and (now_ts - in_flight_ts > 0.150):
                    log.warning("MediaPipe in_flight watchdog timeout (>150ms) — resetting in_flight state.")
                    in_flight = False
                ...
                if not in_flight:
                    ...
                    try:
                        ...
                        log.warning(f"MediaPipe detect_async exception: {async_err}")
                        in_flight = False
                    ...
                    while True:
                        ...
                        if lm:
                            try:
                                ...
                                log.warning(f"Frame processing error: {proc_err}")
                                processor.miss()
                        else:
                            processor.miss()
                except queue.Empty:
